...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const abasAsciiFallback = '[ascii-art.txt unavailable]';
    ...
    // Замени на URL Web App после развёртывания GAS (см. google-apps-script.gs)
    ...
    async function getAbasAscii() {
        if (cachedAbasAscii !== null) return cachedAbasAscii;

        try {
            const response = await fetch('ascii-art.txt');
            if (!response.ok) throw new Error('ascii-art.txt unavailable');
            const text = (await response.text()).trimEnd();
            cachedAbasAscii = text || abasAsciiFallback;
        } catch {
            cachedAbasAscii = abasAsciiFallback;
        }

        return cachedAbasAscii;
    }

    async function renderAbasEasterEgg() {
        addLine('<span class="output-muted">yakutsk folklore module loaded</span>');
        const ascii = await getAbasAscii();
        addLine(`<pre class="easter-ascii">${escapeHtml(ascii)}</pre>`);
    }
